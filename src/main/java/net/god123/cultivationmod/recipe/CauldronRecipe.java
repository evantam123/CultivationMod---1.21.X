...
public record CauldronRecipe(Ingredient inputItem, ItemStack output) implements Recipe<CauldronRecipeInput> {
    ...
    @Override
    public boolean matches(CauldronRecipeInput cauldronRecipeInput, Level level) {
        if (level.isClientSide()) {
            return false;
        }

        return inputItem.test(cauldronRecipeInput.getItem(0));
    }

    @Override
    public ItemStack assemble(CauldronRecipeInput cauldronRecipeInput, HolderLookup.Provider provider) {
        return output.copy();
    }
    ...
    public static class Serializer implements RecipeSerializer<CauldronRecipe> {
        public static final MapCodec<CauldronRecipe> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group(
                Ingredient.CODEC_NONEMPTY.fieldOf("ingredient").forGetter(CauldronRecipe::inputItem),
